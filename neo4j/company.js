const neo4j = require('neo4j-driver');

function createDriver() {
    const driver = neo4j.driver(
        'neo4j+s://21f49612.databases.neo4j.io',
        neo4j.auth.basic('neo4j', 'Jay2833748!')
    );
    return driver;
}

const driver = createDriver();
const session = driver.session();

const limit = 10; // the limit value can be set to any number you want
var graph = {
    nodes : [
    ],
    links : [
    ]
}

const query=`
    match (c:Customer)
    return c.identity as id, c.lastname as lastname, c.pagerank as pagerank, c.community as community
    order by c.pagerank desc 
    LIMIT $limit
`;
const query2 = `
    MATCH (c:Customer)
    WITH c ORDER BY c.pagerank DESC LIMIT $limit
    with COLLECT(c.identity) AS c_ids
    UNWIND c_ids AS c_id with c_id, c_ids
    MATCH (i:Customer_INTER) WHERE i.from = c_id
    UNWIND keys(apoc.convert.fromJsonMap(i.map)) AS c2_id
    MATCH (c2:Customer) WHERE c2.identity = toInteger(c2_id) AND c2.identity <> c_id and c2.identity in c_ids WITH c_id,  c2_id,apoc.convert.fromJsonMap(i.map)[c2_id] AS weight
    RETURN c_id as from, toInteger(c2_id) as to, weight
`;

session.run(query, { limit: neo4j.int(limit) })
    .then(result => {
        result.records.forEach(record => {
            const id = record.get('id');
            const lastname = record.get('lastname');
            const pagerank = record.get('pagerank');
            const community = record.get('community');
            const node = {
                id: id.toNumber(),
                lastname: lastname,
                pagerank: pagerank,
                community: community.toNumber()
              };
            graph.nodes.push(node);
            //console.log(`id: ${id}, lastname: ${lastname}, pagerank: ${pagerank}, community: ${community}`);
        });
    })
    .catch(error => {
        console.error(error);
      })
    .finally(() => {
        console.log(typeof(graph.nodes[0]));
        console.log("dsf");
        console.log(graph.nodes[0]);
        console.log(graph.nodes[0]['id']);
        console.log(typeof(graph.nodes[0]['lastname']));
        session.close();
    });

const session2 = driver.session(); 
session2.run(query2, { limit: neo4j.int(limit) })
    .then(result => {
        result.records.forEach(record => {
            const from = record.get('from');
            const to = record.get('to');
            const weight = record.get('weight');
            //console.log(`From: ${from}, To: ${to}, weight: ${weight}`);
            const link = {
                source: from.toNumber(),
                target: to.toNumber(),
                weight: weight.toNumber()
              };
            graph.links.push(link);
        });
    })
    .catch(error => {
        console.error(error);
      })
    .finally(() => {
        console.log("Sdfd");
        console.log(graph.links[0]);
        session2.close();
        driver.close();
    });

async function getNodeRecords(query, limit) {
    const driver= createDriver();
    const session = driver.session();
    try {
        const result = await session.run(query, { limit: neo4j.int(limit) });
        const records = result.records.map(record => ({
            id: record.get('id').toNumber(),
            lastname: record.get('lastname'),
            pagerank: record.get('pagerank'),
            community: record.get('community').toNumber()
        }));
        return records;
    } catch (error) {
        return Promise.reject(error);
    }
}

async function getEdgeRecords(query, limit) {
    const driver= createDriver();
    const session = driver.session();
    try {
        const result = await session.run(query, { limit: neo4j.int(limit) });
        const records = result.records.map(record => ({
            source: record.get('from').toNumber(),
            target: record.get('to').toNumber(),
            weight: record.get('weight').toNumber()
        }));
        return records;
    } catch (error) {
        return Promise.reject(error);
    }
}
          
          
module.exports = {getNodeRecords, getEdgeRecords};
